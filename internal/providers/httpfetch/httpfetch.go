...
func (p Provider) Extract(ctx context.Context, req core.ExtractRequest) (core.ExtractResponse, error) {
	...
	for hop := 0; ; hop++ {
		...
		// text/plain: preserve textual content without HTML conversion.
		// HTML: extract text + title via htmlToText. Both paths cross the
		// untrusted-content guard before returning data to an agent.
		var content string
		var title string
		var safety core.ContentSafetyReport
		if ct == "text/plain" || strings.HasPrefix(ct, "text/plain") {
			cleaned, textSafety := core.ProtectUntrustedText(string(bodyBytes))
			content = cleaned
			safety = textSafety
		} else {
			text, extractedTitle := htmlToText([]byte(bodyBytes))
			title = extractedTitle
			cleaned, textSafety := core.ProtectUntrustedText(text)
			content = cleaned
			safety = core.MergeContentSafety(core.ScanRawHTMLContentSafety(bodyBytes), textSafety)
		}
